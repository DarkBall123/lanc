...
private _handle = [{
	...
	if (_mode isEqualTo "MANUAL") then {
		private _rawInput = _projectile getVariable ["lancet_geran_manualInput", [0, 0]];
		...
		private _smoothedInput = _projectile getVariable ["lancet_geran_manualInputSmoothed", [0, 0]];
		...
		_projectile setVariable ["lancet_geran_manualInputSmoothed", _smoothedInput];
		_projectile setVariable ["lancet_geran_turnSpeed", 0];
			...
			if (!isNull _target && {alive _target}) then {
				if (diag_tickTime >= _nextTrackCheck) then {
					...
					private _confidence = [
						...
						_thermal,
						_detectionRange
					] call lancet_fnc_getGeranConfidence;

					_projectile setVariable ["lancet_geran_targetVisible", _confidence >= 0.4];
					_projectile setVariable ["lancet_geran_nextTrackCheck", diag_tickTime + 0.25];
				};

				if (_projectile getVariable ["lancet_geran_targetVisible", false]) then {
					private _offsetModel = _projectile getVariable [
						"lancet_geran_targetOffsetModel",
						[0, 0, 0]
					];
		...
		if (_desiredDirection isNotEqualTo [0, 0, 0]) then {
			...
			_projectile setVariable ["lancet_geran_turnSpeed", _turnSpeed];
